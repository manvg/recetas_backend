...
public interface RecetaRepository extends JpaRepository<Receta, Integer> {
   ...
   @Query("SELECT r FROM Receta r WHERE " +
           ...
           "(r.tipo_cocina LIKE %:tipoCocina% OR :tipoCocina IS NULL) AND " +
           "(r.pais_origen LIKE %:paisOrigen% OR :paisOrigen IS NULL) AND " +
           "(r.dificultad LIKE %:dificultad% OR :dificultad IS NULL)")
    List<Receta> buscarRecetas(@Param("nombre") String nombre,@Param("descripcion") String descripcion,@Param("tipoCocina") String tipoCocina,@Param("paisOrigen") String paisOrigen,@Param("dificultad") String dificultad);
}